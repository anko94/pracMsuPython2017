...
def checkIfOneTriangleLiesInAnother(triangle1: Triangle, triangle2: Triangle, plane1: Plane, plane2: Plane, alg: Algebra):
    lies = 0
    sum = 0
    ...
    if planeRelationship == 0:
        ...
        for i in range(len(straights1)):
            for j in range(len(straights2)):
                ...
                if abs(cos + 1) < accuracy or abs(cos - 1) < accuracy:
                    ...
                    if point2 != None and abs(alg.checkIfPointBetween4Another(point2, straights1[i].point0, straights1[i].point2,
                                                           straights2[j].point0, straights2[j].point2) - 1) < accuracy:
                        isTrianglesIntersect = 1
                        break
            if isTrianglesIntersect == 1:
                break
        if isTrianglesIntersect == 0:
            isTrianglesIntersect = checkIfOneTriangleLiesInAnother(triangle1, triangle2, plane1, plane2, alg)
        ...
        if c1 == 1 and c2 == 1:
            ...
            for i in range(len(points) - 1):
                point3 = points[i]
                point4 = points[i + 1]
                if abs(point3.x - point4.x) < accuracy and abs(point3.y - point4.y) < accuracy and abs(
                                point3.z - point4.z) < accuracy and point3.mark != point4.mark:
                    isTrianglesIntersect = 1
                    break
            if isTrianglesIntersect == 0:
                markq = points[0].mark
...
def drawTriangle(list1, list2):
    fig = plt.figure()
    ax = Axes3D(fig)
    x = [list1[0][0], list1[0][1], list1[0][2]]
    y = [list1[1][0], list1[1][1], list1[1][2]]
    z = [list1[2][0], list1[2][1], list1[2][2]]
    x1 = [list2[0][0], list2[0][1], list2[0][2]]
